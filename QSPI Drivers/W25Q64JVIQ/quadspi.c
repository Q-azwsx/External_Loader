...
/* USER CODE END 0 */
//it may work on all w25q64jv devices, but only tested by w25q64jviq
...
uint8_t CSP_QUADSPI_Init(void) {
	...
	volatile uint32_t delay = HAL_RCC_GetHCLKFreq() / 1000;
	while (delay--) __NOP();
...
uint8_t QSPI_Configuration(void) {
	//some codes here are not necessary
	QSPI_CommandTypeDef sCommand;
...
uint8_t CSP_QSPI_EnableMemoryMappedMode(void) {
	...
	sCommand.DummyCycles = DUMMY_CLOCK_CYCLES_READ_QUAD_IO;
...
uint8_t QSPI_ResetChip() {
	...
	//the w25q64 device only needs 1 line reset, a short delay is needed after reset chip
	return HAL_OK;
}
